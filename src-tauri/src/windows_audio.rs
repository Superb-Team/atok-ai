    ...
    fn recording_thread_fn(output_path: PathBuf, is_recording: Arc<Mutex<bool>>) -> Result<()> {
        ...
        unsafe {
            ...
            loop {
                        ...
                        if !is_silent {
                            if mic_bits == 32 {
                                for chunk in audio_data.chunks_exact(4) {
                                    ...
                                    let mic_sample =
                                        (float_val * 32767.0 * 1.5).clamp(-32768.0, 32767.0) as i16;
                                ...
                                for chunk in audio_data.chunks_exact(2) {
                                    ...
                                    // Boost mic volume by 1.5x
                                    let mic_sample =
                                        ((sample as i32 * 3) / 2).clamp(-32768, 32767) as i16;
                                    mic_buffer.push(mic_sample);
                                }
                            }
                        } else {
                ...
                // Mix desktop and mic buffers with gain staging + soft limiter
                let min_len = desktop_buffer.len().min(mic_buffer.len());
                if min_len > 0 {
                    let system_gain = 10.0_f32.powf(6.0 / 20.0); // +6dB
                    for i in 0..min_len {
                        let desktop_f = desktop_buffer[i] as f32 / 32767.0;
                        let mic_f = mic_buffer[i] as f32 / 32767.0;

                        let boosted_sys = desktop_f * system_gain;
                        let sum = (mic_f + boosted_sys) * 0.5;

                        let limited = if sum.abs() > 0.95 {
                            sum.signum()
                                * (0.95 + (1.0 - (-(sum.abs() - 0.95) * 20.0).exp()) * 0.05)
                        } else {
                            sum
                        };

                        sample_buffer.push((limited.clamp(-0.98, 0.98) * 32767.0) as i16);
                    }
                ...
                while sample_buffer.len() >= chunk_size {
                    ...
                    let bytes_written =
                        mp3_encoder.encode(input, &mut mp3_buffer).map_err(|e| {
            ...
            if min_len > 0 {
                for i in 0..min_len {
                    ...
                    let mixed = ((desktop_sample * 7 / 10) + (mic_sample * 8 / 10))
